...
- 标注撤销/重做
- 编辑后的截图另存为、复制和完成编辑
- 截图编辑器内翻译当前截图选区，支持 OCR 原文校对、截图与译文并排查看、目标语言选择、重试、复制和悬浮结果面板
...
截图编辑器中的“翻译”会先使用 macOS Vision 在本地 OCR 当前选区的最终图像，因此箭头、文字和马赛克会按当前状态参与识别；识别结果会在发送前展示并允许手动修正，确认后才会把文字发送给设置中配置的 OpenAI-compatible 文本模型进行翻译，不要求模型支持多模态。翻译结果仅在内存中显示，不会写入截图历史，并支持在“截图翻译”设置卡片中选择目标语言。
