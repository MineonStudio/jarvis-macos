...
这样做的效果是：**屏幕录制、辅助功能、麦克风和摄像头授权在同渠道更新时会保留**——TCC 把授权记在代码身份上，更新时用同一张证书重签即可沿用。首次启动时也可以在权限页完成本机签名；签名完成后再授权，后续同渠道更新就能沿用。信任设置仅对本机生效，只影响这张密钥签出的代码，而密钥从不离开这台 Mac。
...
```bash
JARVIS_VERSION="1.4.9" JARVIS_BUILD="347" ./build_app.sh
```
...
版本号会显示在设置页的“版本与更新”区域。正式发布时使用 `v主版本.次版本.修订版本` 标签，并通过 `./package_release.sh` 生成 GitHub Release 所需的直接下载 ZIP、DMG、应用内更新 ZIP 和 Ed25519 签名清单。更新由用户手动检查；发现新版后 Jarvis 会自动下载、校验并替换安装。ZIP App 和 DMG 是两个安装来源，但应用内升级共用同一条替换流程，并保留当前来源标记。切换安装来源时会清除屏幕录制、辅助功能、麦克风和摄像头的旧授权，再由新来源重新申请。为保证同渠道更新保留授权，必须先完成一次本机签名；更新器无法使用该签名身份时会停止并显示具体原因，不会降级为 ad-hoc 签名安装。

用 `install.sh` 安装的副本会在首次启动前完成本机签名。通过 ZIP App 或 DMG 安装的副本也需要在权限页先完成这一步，再授予四项系统权限。钥匙串访问属于 macOS 的另一套授权，应用二进制更新后仍可能重新询问。
