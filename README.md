...
截图编辑器中的“翻译”会先使用 macOS Vision 在本地 OCR 当前选区的最终图像，因此箭头、文字和马赛克会按当前状态参与识别；随后自动把 OCR 文字块发送给设置中配置的 OpenAI-compatible 文本模型进行翻译，不要求模型支持多模态。每个 OCR 文字块都使用稳定标记单独翻译，原文和译文保持一一对应；翻译完成后会在对应原文区域应用高斯模糊，并根据该区域的原文行高与自然宽度估算匹配字号，同时采样原文前景颜色后回填译文。如果译文在对应原区域内放不下，会逐步缩小字体；模型未返回完整分块结果时不会渲染不确定的统一译文。翻译不会弹出独立面板，结果直接回到截图编辑器中，并可继续确认、复制或保存。
